...
float rollThreshold = 13.5; // Threshold for the roll angle change
float accumalatedRateOfChange = 0.0;
...
void loop(){
  ...
  accumalatedRateOfChange += rollRateOfChange;
  ...
  if (acceleration > 1.4) {
    ...
    for (int i = 0; i < 5; i++) {
      if(dataPoints[i]<sensitiveLevel){
        if (accumalatedRateOfChange >= rollThreshold) {
  ...
  if (millis() - monitoringRateOfChangeTime >= resettingRateOfChangeInterval) {
    accumalatedRateOfChange = 0;  //Reset the total change of orientation (roll)
