...
float rollThreshold = 20.0; // Threshold for the roll angle change
float accumalatedRateOfChangeRoll = 0.0;
float previousPitchAngle = 0.0; // Variable to store the previous pitch angle
float pitchThreshold = 8.0; // Threshold for the pitch angle change
...
void checkUserInput(time_t currentTime, String stringTime){
  ...
  while (millis() - startTime < 15000) {
    ...
    if (pressDurationStopFall < 15000 && pressDurationStopFall >= 1500) {
      fallflag = false;
      digitalWrite(buzzerPin,LOW);
      break;
    } else {
...
void loop(){
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if(dataPoints[i]<sensitiveLevel){
        ...
        if (accumalatedRateOfChangeRoll >= rollThreshold || accumalatedRateOfChangePitch >= pitchThreshold) {
          delay(200);
          //check for any movements for 3 seconds
          for (int i = 0; i < 56; i++){
  ...
  if (buttonPressedFlag) {
    ...
    if (pressDuration >= 15000) {
      ...
      setEmergency();
    } else if (pressDuration < 5000 && pressDuration >= 1500) {
      // Short press action (less than 5 seconds and more than 1.5 seconds). To prevent user's from accidentally stopping resetting the fall status
