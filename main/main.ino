...
bool noMovementDetected = true; //Variable to check if the user falls after a fall
...
float rollThreshold = 18.0; // Threshold for the roll angle change
float accumalatedRateOfChangeRoll = 0.0;
float previousPitchAngle = 0.0; // Variable to store the previous pitch angle
float pitchThreshold = 7.0; // Threshold for the pitch angle change
...
void updateLocation() {
  ...
  if (gps.location.isValid()) {
    // Update location if the GPS data is valid
    if (gps.location.isUpdated()) {
      ...
      //Serial.println("GPS data is not updated.");
    }
    gpsStatus = true; // Set to true if data is valid, regardless of update
  } else {
...
void checkUserInput(time_t currentTime, String stringTime){
  digitalWrite(buzzerPin,HIGH);
  fallflag = true;

  //Monitor the time during potential fall detected. Used in the updating of cloud firestore values every second
  unsigned long startTime = millis();
  unsigned long lastUpdateTime = startTime;

  // waits 15 second to check if the fall is false positive
  while (millis() - startTime < 15000) {
    if (!isnan(AccX) && !isnan(AccY) && !isnan(AccZ)) {
      // Update values to Firestore every second
    ...
    // check to see if the user presses the button wihthin 15s indicaing a false positive
    unsigned long pressDurationStopFall = millis() - pressStartTime;
    if (pressDurationStopFall < 15000 && pressDurationStopFall >= 1000) {
      fallflag = false;
      digitalWrite(buzzerPin,LOW);
      break;
    } else {
...
void setup(){
  ...
  readFirestore(FIREBASE_PROJECT_ID, userId, true, false);  // Read username and sesitive fields initialize Firestore fields
  readFirestore(FIREBASE_PROJECT_ID, userId, false, false);  // Read the initial sensitive value from Cloud Firestore
}
...
void loop(){
  ...
  if (acceleration > 1.6) {
    ...
    resetFlag = false;  //Do not reset the accumalated orientation
    for (int i = 0; i < 5; i++) {
      //check if there is any freefall
      if(dataPoints[i]<sensitiveLevel){
        //check if the device orientation changes
        if (accumalatedRateOfChangeRoll >= rollThreshold || accumalatedRateOfChangePitch >= pitchThreshold) {
          delay(100);
          //check for any movements for 2 seconds
          for (int i = 0; i < 30; i++){
            gyro_signals();  //update the acceleration
            if (acceleration < 0.8 || acceleration > 1.2) {  //to check if the user is not moving
              //Serial.println("movement");  Check if the movement is detected
              noMovementDetected = false;  //user moved
              break;  // Exit the inner loop if a fall is detected
            } else {
                delay(50);
            }
          }
          if (noMovementDetected) {  // Check if fallIssue is true
            fallIssue = true;  // A fall is detected
            break;  // Exit the outer loop if a fall is detected
          } else {
            noMovementDetected = true;  // reset the movement detected flag 
            break;  // Exit the outer loop if a fall is detected
          }
        }
      } 
  ...
  if(fallIssue){
    fallIssue = false;
    checkUserInput(currentTime, stringTime);  // Checks if the user pressed the button(Indication that is is a false positive)
    if(fallflag == true){
      fallStatus = true;
  ...
  if (millis() - monitoringStartTime >= sendingInterval) {
    ...
    readFirestore(FIREBASE_PROJECT_ID, userId, false, gpsStatus);  // Update the sensitive value from Cloud Firestore
  ...
  if (buttonPressedFlag) {
    ...
    if (pressDuration >= 15000) {
      restartESPWifi();
    } else if (pressDuration < 15000 && pressDuration >= 3000) {
      // Long press action (less than 15 seconds and more than 3 seconds)
      setEmergency();
    } else if (pressDuration < 3000 && pressDuration >= 1500) {
      // Short press action (less than 3 seconds and more than 1.5 seconds). To prevent user's from accidentally stopping resetting the fall status
